<template>
  <div class="chat-box flex flex-col justify-between flex-1">
    ...
    <div class="overflow-y-auto pe-[10px] space-y-4 max-h-[540px]">
        ...
        <div v-for="(msg, index) in messages" :key="index">
          ...
          <div v-if="msg.direction === 'inbound'" class="flex gap-[15px]">
            ...
            <div>
              ...
              <p class="text-[#5B605C] text-[14px] bg-[#F2F7FB] p-[12px] rounded-[16px] rounded-ss-none ms-[10px] mb-[7px] inline-block">
                {{ msg.text }}
              </p>

              <!-- Attachments Preview -->
              <div v-if="msg.attachments?.length" class="ms-[10px] mb-[7px] grid grid-cols-2 gap-3">
                <div v-for="(attachment, index) in msg.attachments" :key="index" class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                  <a target="_blank" v-if="isImage(attachment)" :href="attachment">
                    <img :src="attachment" alt="Attachment" class="w-full h-[100px] object-cover rounded-xl" />
                  </a>
      ...
      <template v-else>
        <p class="text-center text-[#C6C6C7] text-sm mt-10">{{ t("no_messages_yet") }}</p>
      </template>
    </div>
    ...
    <div v-if="!isClosed" class="flex items-center gap-2 mt-4">
      <div class="flex-1 relative h-[50px] rounded-[13px] overflow-hidden">
        <input v-model="newMessage" type="text" :placeholder="t('type_a_message')" class="ps-[23px] w-full h-full text-sm outline-none border border-[#D8D8D8] bg-white text-[#959595] font-light text-[14px] leading-normal" />
        <button class="absolute h-full end-0 px-[19px] cursor-pointer" @click="attach">
          <AttachIcon />
        </button>
      </div>
      <button :disabled="!newMessage.trim()" class="w-[48px] h-[50px] rounded-[13px] bg-[#EBE4DF] flex items-center justify-center text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed rtl-rotate" @click="send">
        <SendIcon />
      </button>
    </div>
    ...
    <div v-else class="text-center mt-6">
      ...
      <p class="text-[#C6C6C7] font-medium text-[18px] leading-normal">{{ t("chat_closed_title") }}</p>
      <p class="text-[#C6C6C7] font-[400] text-[15px] leading-normal">{{ t("chat_closed_subtitle") }}</p>
    </div>
  </div>
</template>
...
<script setup>
import { ref } from 'vue'
import { useToast, useApi, useI18n } from '#imports'
...
async function handleFileUpload(event) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    toast.add({ title: t("file_size_error"), color: 'error' })
    return
  }
  ...
  try {
    ...
    if (data.value?.status) {
      toast.add({ title: t("file_upload_success"), color: 'success' })
      emit('refresh')
    } else {
      toast.add({ title: t("file_upload_failed"), color: 'error' })
    }
  } catch (err) {
    toast.add({ title: t("file_upload_error"), color: 'error' })
  }
}
...
async function send() {
  ...
  try {
    ...
    if (data.value?.status) {
      toast.add({ title: t("message_sent"), color: 'success' })
      emit('refresh')
      newMessage.value = ''
      selectedFiles.value = []
      fileInput.value.value = ''
    } else {
      toast.add({ title: t("message_failed"), color: 'error' })
    }
  } catch (err) {
    toast.add({ title: t("message_error"), color: 'error' })
  }
}
